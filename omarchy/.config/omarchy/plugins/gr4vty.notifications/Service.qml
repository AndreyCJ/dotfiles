...
Item {
  ...
  // stacked toast cards, but only the window on the main monitor is ever
  // mapped (see isMainMonitor). Layer is Overlay, exclusionMode Ignore, no
  ...
  Variants {
    ...
    PanelWindow {
      ...
      screen: modelData
      visible: popupModel.count > 0 && service.isMainMonitor(modelData)
